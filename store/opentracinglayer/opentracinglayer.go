...
func (s *OpenTracingLayerChannelStore) IncrementMentionCount(channelID string, userIDs []string, updateThreads bool, isRoot bool) error {
	origCtx := s.Root.Store.Context()
	...
	defer span.Finish()
	err := s.ChannelStore.IncrementMentionCount(channelID, userIDs, updateThreads, isRoot)
	if err != nil {
		span.LogFields(spanlog.Error(err))
		ext.Error.Set(span, true)
	}
